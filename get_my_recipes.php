...
$current_chef_id = isset($_GET['current_chef_id']) ? (int)$_GET['current_chef_id'] : 0;
...
$query = "SELECT r.RecipeID, r.title, r.image, r.difficulty_lvl, 
          c.name AS cuisine, m.name AS meal_type,
          u.name AS chef_name,
          (SELECT AVG(rating) FROM rate WHERE recipe_id = r.RecipeID AND status = 'published') AS avg_rating
          FROM recipe r
          JOIN cuisine_type c ON r.cuisine_typeID = c.CuisineID
          JOIN meal_types m ON r.meal_typesID = m.MealID
          JOIN chef ch ON r.chefs_id = ch.ChefID
          JOIN user u ON ch.user_id = u.UserID
          WHERE $filter_sql
...
if (mysqli_num_rows($result) > 0) {
    while ($recipe = mysqli_fetch_assoc($result)) {
        ...
        echo '<a href="edit_recipe.php?id=' . $recipe['RecipeID'] . '" class="btn btn-sm btn-primary me-2"><i class="fas fa-edit me-1"></i> Edit</a>';
        echo '<a href="delete_recipe.php?id=' . $recipe['RecipeID'] . '" class="btn btn-sm btn-outline-danger"><i class="fas fa-trash me-1"></i> Delete</a>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
    ...
    echo '<div class="alert alert-info">No recipes found matching your criteria. <a href="chefAddRecipe.php">Add a new recipe!</a></div>';
    echo '</div>';
}
?>